...
app = FastAPI(title="COSC 3506 Phase 4 API", version="4.0.0")
...
JWT_SECRET = os.getenv("JWT_SECRET", "phase4-development-secret-key-123456")
...
def get_current_user(
    ...
    try:
        ...
        return {"username": str(username).strip(), "role": str(role).strip().lower()}
...
def require_owner_or_admin(student_id: str, current_user: dict):
...
def startup_event():
    logger.info("COSC 3506 Phase 4 API started successfully.")

    logger.info(
        "Admin role currently stored as: %s", users.get("admin", {}).get("role")
    )


@app.get("/")
def root():
    return {"message": "COSC 3506 Phase 4 API is running", "version": "4.0.0"}
...
def register(body: AuthBody):
    ...
    users[username] = {"password_hash": hash_password(body.password), "role": "student"}
...
async def import_catalog(
    ...
    logger.info("Catalog import requested by admin user: %s", current_user["username"])
    ...
    logger.info("Catalog imported successfully. Courses loaded: %s", len(catalog))
...
async def import_history(
    ...
    if student_id in students:
        existing_plan = students[student_id].get("plan", [])
...
def update_history(
    student_id: str, body: HistoryBody, current_user: dict = Depends(get_current_user)
):
    require_owner_or_admin(student_id, current_user)
...
def delete_history(student_id: str, current_user: dict = Depends(get_current_user)):
    require_owner_or_admin(student_id, current_user)
...
def create_plan(
    student_id: str, body: PlanBody, current_user: dict = Depends(get_current_user)
):
    require_owner_or_admin(student_id, current_user)
...
def delete_plan(student_id: str, current_user: dict = Depends(get_current_user)):
    require_owner_or_admin(student_id, current_user)
...
def get_plan(student_id: str, current_user: dict = Depends(get_current_user)):
    require_owner_or_admin(student_id, current_user)

    if student_id not in students:
        raise HTTPException(status_code=404, detail="Student not found")

    return {"student_id": student_id, "planned_courses": students[student_id]["plan"]}
...
def audit_report(
                ...
                timeline_errors[planned_term].append(
                    {
                        ...
                        "message": ("Missing prerequisite: " f"{prerequisite}"),
                    }
                )
    ...
    if has_issues:
        status_value = "failed" if strict else "warning"
...
def get_recommendations(
    ...
    for course in history:
        if course["status"] == "Completed":
            completed_courses.add(normalize_course_code(course["course_code"]))
    ...
    for course_code, course in catalog.items():
        ...
        for prerequisite in course["prerequisites"]:
            ...
            if prerequisite_code in graph and normalized_course in graph:
                graph[prerequisite_code].append(normalized_course)

                indegree[normalized_course] += 1

    queue = [course_code for course_code in indegree if indegree[course_code] == 0]
    ...
    while queue:
        ...
        recommended_pathway.append(
            {"term": f"Term {term_number}", "courses": term_course_codes}
        )
    ...
    return {"student_id": student_id, "recommended_pathway": (recommended_pathway)}
