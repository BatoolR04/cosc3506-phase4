...
def get_recommendations(
    ...
    # Include only courses that have not already been completed.
    ...
    # Build prerequisite edges for unfinished prerequisites.
    ...
    # Start with courses that have no unfinished prerequisites.
    queue = [
        course_code
        for course_code, degree in indegree.items()
        if degree == 0
    ]
    ...
    while queue:
        ...
        for current_course in current_term_courses:
            recommended_pathway.append(
                {
                    "course_code": catalog[current_course]["course_code"],
                    "term": term_number,
                }
            )
